<?php
	...
	$tag = $_GET["tag"];
	$tag = $connection->real_escape_string($tag);
	$_tag = mb_convert_encoding($tag, 'UTF-8', 'UTF-8');
	$_tag = htmlentities($_tag, ENT_QUOTES, 'UTF-8');
	...
	$query = "SELECT * FROM tag WHERE title = '$_tag'";
...
<html lang="en" data-theme="light">
	<head>
		...
		<title><?php echo $_tag ?><?php if(isset($_GET["page"])) {echo " - Page: $_page"; } ?> | Hack Hex</title>
	...
	<body>
		...
		<div class="container maxWidth1132 margin30" style="display: flex;">
			<div class="items">
				<?php 
							$tagId = $row["id"];
							echo "<h1 class=\"desc\" style=\"font-size:1.8rem;\">" . html_entity_decode($row["title"]) . "</h1>";
						}
					}
					...
					$query = "SELECT * FROM question q WHERE q.id IN (SELECT questionId FROM qtrelation qtr WHERE tagId = '$tagId') ORDER BY id DESC LIMIT $pStart, 10";
					$result = $connection->query($query);
				...
				<?php
						...
						if(isset($_GET["page"]))
						{
							...
							if($_page==1) {
								echo '<a href="https://' . $_SERVER['HTTP_HOST'] . '/t/' . $_tag . '"><- Previous Page</a>';
							} else {
								echo '<a href="https://' . $_SERVER['HTTP_HOST'] . '/t/' . $_tag . '/' . ($_page-1) . '"><- Previous Page</a>';
							}
							
							echo '<a href="https://' . $_SERVER['HTTP_HOST'] . '/t/' . $_tag . '/' . ($_page+1) . '" class="next">Next Page -></a>';
                            echo '</div>';
						} else {
                            echo '<div class="nav-link">';
							echo '<a href="https://' . $_SERVER['HTTP_HOST'] . '/t/' . $_tag . '/1" class="next">Next Page -></a>';
                            echo '</div>';
						}
